...
from ..manifolds.stereographic.math import arsinh, artanh
from ..utils import *
...
class UnidirectionalPoincareMLR(nn.Module):
    ...
    def __init__(self, feat_dim, num_outcome, bias=True, ball=None):
        ...
        weight = torch.empty(feat_dim, num_outcome, dtype=torch.float64).normal_(
            mean=0, std=(self.feat_dim) ** -0.5 / self.ball.c.data.sqrt())
        self.weight_v = nn.Parameter(weight)
        self.bias = nn.Parameter(torch.empty(num_outcome, dtype=torch.float64), requires_grad=bias)
    ...
    def forward(self, x):
        ...
        return unidirectional_poincare_mlr(x, w_norm, self.weight_v / w_norm, self.bias, self.ball.c)
