...
from ..manifolds.stereographic.math import (
    _mobius_add,
    _mobius_scalar_mul,
    _project,
    weighted_midpoint
...
class PoincareLinear(nn.Module):
    def __init__(self, in_dim, out_dim, out_split=1, bias=True, ball=None, gain=1.):
        ...
        weight = torch.empty(in_dim, out_dim, dtype=torch.float64).normal_(
        ...
        self.bias = nn.Parameter(torch.empty(out_dim, dtype=torch.float64), requires_grad=bias)
    ...
    def forward(self, x):
        x = poincare_linear(
            x, 
            self.weight_v.norm(dim=0).clamp_min(1e-15), 
